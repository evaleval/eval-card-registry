...
| Metric bounds | `min_score` / `max_score`; `.inf` / `-.inf` = unbounded on that side by definition, `null` = not stated; serialised as `"Infinity"` / `"-Infinity"` on the wire | perplexity `[1.0, .inf]` |
