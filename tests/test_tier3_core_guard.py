"""The Tier-3 inferred-seed generator's mint/guard logic must NOT emit a
canonical id that normalized-collides with a curated `core.yaml` canonical under
a DIFFERENT id.

OFFLINE + non-destructive: loads the script via importlib and exercises the guard
FUNCTIONS directly (`build_core_norm_index` / `core_steals`). It never invokes the
script's `main()`, never touches fixtures, and never runs the network.

Guarantee under test: HF/curated canonicals WIN id + casing. A generated row that
would collide (normalized, via the resolver `normalize`) with a curated core
canonical under a different id is the SAME model — it must defer to the curated id
(skip the mint so the raw resolves to curated), never mint a colliding twin.
"""
    ...
    "entries": [
        {
            ...
            # This curated id IS its own real HF repo (hf_id == id).
...
def test_core_collision_is_SKIPPED_not_minted_as_inferred_twin(mod):
    """Merge, never dup: a residual raw whose minted id normalized-collides with a
    curated core canonical is the SAME model as that curated entry — the policy must
    SKIP it (drop the row so the raw resolves to the curated canonical), NOT mint a
    `{cid}-inferred` duplicate that splits one model into two and shadows the
    curated entry.
    ...
    # The decision is SKIP — NOT 'inferred' and NOT 'mint'.
...
def test_guard_never_rewrites_id_equal_to_its_hf_id(mod):
    """The guard never rewrites an id whose metadata.hf_id equals that id. The
    curated core id `Foo/Bar-7B` (hf_id == id) is the WIN target — `core_steals`
    returns False for it, so it is never suffix-disambiguated."""
    idx = mod.build_core_norm_index(CORE_DOC)
    assert mod.core_steals("Foo/Bar-7B", idx) is False
